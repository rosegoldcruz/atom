...
export function useArbitrageContracts() {
  // Mock connection for now - replace with actual wallet connection later
  ...
  useEffect(() => {
    const initContracts = async () => {
      ...
      try {
        // For ethers v6, we need to create a BrowserProvider first
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      setStats({
        ...
        totalProfitUSD: parseFloat((totalProfitUSD / BigInt(10**18)).toString()),
        successRate,
        dailyProfit: parseFloat((totalProfitUSD / BigInt(10**18)).toString()) // Simplified
      });
    } catch (err) {
  ...
  const fetchOpportunities = useCallback(async () => {
    ...
    try {
      ...
      const formattedOpportunities: ArbitrageOpportunity[] = alerts.map((alert: any, index: number) => ({
        ...
        estimatedProfit: parseFloat((alert.estimatedProfit / BigInt(10**18)).toString()),
    ...
    try {
      ...
      const pricePromises = tokenAddresses.map(async (token) => {
        try {
          ...
          return {
            token,
            chainlinkPrice: parseFloat((price / BigInt(10**18)).toString()),
